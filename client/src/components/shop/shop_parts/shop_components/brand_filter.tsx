...
const BrandFilter: FC<BrandFilterProps> = ({
  brands, products, filters, filterByBrandId, resetBrandFilter,
}): JSX.Element => (
  <div className="relative h-56 w-full overflow-y-auto rounded-md bg-white/50 p-2">
    <h1 className="filters-title mb-2">Filter by Brand</h1>
    <ResetFilterIcon
      ...
      className={`absolute left-3 top-1 size-8 cursor-pointer rounded-full transition-transform duration-500 ease-in-out ${
        !filters.triggeredFilters.brand
          ? 'bg-gray-400 fill-gray-900 '
    ...
    {brands?.map(({ _id = '', name }) => (
      ...
      >
        <Checkbox
          ...
          checked={filters.brand?.includes(_id) ?? false}
          onChange={() => filterByBrandId(_id)}
          color="blue"
          className="-z-10"
        />
